...
{
    private const string TERRIAN_TILEMAP_OBJ_NAME = "Terrain Tilemap";
    ...
    {
        tileMapObjName = TERRIAN_TILEMAP_OBJ_NAME;
        ...
        for (int i = 0; i < allTileObjs.Count; i++)
        {
            if (tempTileY.IsEquals(allTileObjs[i].transform.localPosition.y) == false)
            {
                mapCellSize.x = i;
                break;
            } // if : 첫번쨰 타일의 y 좌표와 달라지는 지점 전까지 맵의 가로 셀 크기이다.
        }
        ...
        //{ x축 상의 두 타일과 y축 상의 두 타일 사이의 롵컬 포지션으로 타일 갭을 연산한다.
    ...
    private void Start()
    {
        ...
        float correctChangerPercentage = allTileObjs.Count * (CHANGE_PERCENTAGE / 100.0f);
        ...
        for (int i = 0; i < allTileObjs.Count; i++)
        {
            if (correctChangerPercentage <= changedTileResult[i]) { continue; }
